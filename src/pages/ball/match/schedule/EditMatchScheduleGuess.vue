...
@Component
export default class EditMatchScheduleGuess extends Vue {
  data() {
    return {
      loading: false,
      editFormRules: {
  ...
  async getDetail() {
    const { matchScheduleGuessGameId } = this.$route.params;
    const res = await ApiSchedule.getMatchGuessGameDetail(matchScheduleGuessGameId);
